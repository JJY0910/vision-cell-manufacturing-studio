...
- `InspectionRunUseCase` persists final Judge, defect summary, per-step timings, Recipe metadata, generated lot ID, source image artifact path, and 2D/3D defect records through `IInspectionResultRepository` after Judge succeeds.
- `InspectionRunUseCase` now writes deterministic source, overlay, and height-map BMP artifacts through `IInspectionArtifactWriter` during Persist Result, then saves the relative artifact paths with the SQLite result row.
- Stop Inspection requests cancellation for the active run token through `InspectionViewModel`.
- The generated BMP overlay is persistence evidence for Offline Debug/reporting workflows; richer live viewport overlay polish remains separate UI work.
