...
- Offline Debug Re-inspect preparation maps source result lot, Recipe, judgment, cycle time, defect, artifact context, and active-vs-historical Recipe policy; `Run Re-inspect` executes and persists an Application metadata comparison without live camera/motion/vision sequence replay
- Offline Debug Re-inspect readiness rows keep metadata comparison, source-image replay, Recipe policy metadata, metadata history persistence, and real sequence execution boundaries visible in XAML/ViewModel tests.
