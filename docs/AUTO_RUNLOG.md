...
## 2026-06-03 14:18 - Local validation passed
- Branch: `feature/fr-203-reports-scope-wording`
- Scope: Align ReportsView and UI QA documentation with FR-203 CSV export and FR-204 lot summary requirements, and update Offline Debug QA wording to reflect metadata comparison without claiming source-image replay or replay persistence.
- Validation target: App HMI XAML tests, Debug/Release build/test, static blocking/code-behind/secret scans, WPF launch smoke, GitHub Actions after PR creation.
- Local validation: App test project Debug build passed; App HMI XAML targeted tests passed with 10 tests. `dotnet restore .\VisionCell.sln`, Debug solution build, Debug solution test, Release solution build, and Release solution test passed. Debug and Release solution tests each passed with 254 tests. `git diff --check` passed with CRLF warnings only; precise blocking wait scan found no `.Result`, `.Wait()`, or `Thread.Sleep()` matches; WPF code-behind scan found no event-handler or blocking-call matches; narrow diff secret scan found no matches; WPF hidden launch smoke passed by starting `dotnet run --project .\src\VisionCell.App\VisionCell.App.csproj -c Debug --no-build` for 6 seconds after discarding an earlier malformed project-path smoke attempt.
- Risks: This slice changes WPF scope wording, XAML QA coverage, and documentation only. It does not implement CSV export, lot summary report generation, source-image replay, replay result persistence, physical HMI acceptance, or real equipment validation.

## 2026-06-03 14:29 - Local validation passed
- Branch: `feature/fr-240-settings-readiness-scope`
- Scope: Surface the read-only Settings runtime scope and Real Hardware readiness gate through `SettingsViewModel` rows and `SettingsView` bindings so operators can see missing adapter/bench validation evidence without enabling real hardware mode.
- Validation target: Settings ViewModel test, App HMI XAML tests, Debug/Release build/test, static blocking/code-behind/secret scans, WPF launch smoke, GitHub Actions after PR creation.
- Local validation: App test project Debug build passed; Settings/HMI targeted App tests passed with 11 tests. `dotnet restore .\VisionCell.sln`, Debug solution build, Debug solution test, Release solution build, and Release solution test passed. Debug and Release solution tests each passed with 255 tests. `git diff --check` passed with CRLF warnings only; precise blocking wait scan found no `.Result`, `.Wait()`, or `Thread.Sleep()` matches; WPF code-behind scan found no event-handler or blocking-call matches; narrow diff secret scan found no matches; WPF hidden launch smoke passed by starting `dotnet run --project .\src\VisionCell.App\VisionCell.App.csproj -c Debug --no-build` for 6 seconds.
- Risks: This slice exposes readiness state only. It does not implement persisted Settings editing, real hardware profile enablement, `RealEquipmentController`, Motion/Camera/PLC adapters, bench validation, physical HMI acceptance, or real equipment validation.

## 2026-06-03 14:42 - Local validation completed with App Control limitation
- Branch: `feature/fr-222-reinspect-readiness-scope`
- Scope: Add read-only Offline Debug Re-inspect readiness rows for metadata comparison, source-image replay, Recipe policy, replay persistence, and real sequence execution so the operator-visible panel distinguishes available simulator metadata comparison from unimplemented or unvalidated replay work.
- Validation target: Offline Debug ViewModel/XAML tests, Debug/Release build/test, static blocking/code-behind/secret scans, WPF launch smoke, GitHub Actions after PR creation.
- Local validation: App test project Debug build passed; Offline Debug/HMI targeted App tests passed with 11 tests. `dotnet restore .\VisionCell.sln`, Debug solution build, and Release solution build passed with 0 warnings/errors. `git diff --check` passed with CRLF warnings only; precise blocking wait scan found no `.Result`, `.Wait()`, or `Thread.Sleep()` matches; WPF code-behind scan found no event-handler or blocking-call matches; narrow diff secret scan found no matches.
- Local App Control limitation: Full Debug solution test, single-node Debug solution test, Release solution test, and WPF hidden launch smoke were blocked before full assertions because Windows Application Control blocked generated assemblies such as `VisionCell.Motion.dll`, `VisionCell.Simulator.dll`, and App/Vision test DLLs with `0x800711C7`. `Unblock-File` on generated source/test DLLs did not clear the local policy block. GitHub Actions must provide the full test and launch/build signal before merge.
- Risks: This slice changes read-only Offline Debug ViewModel/XAML/docs only. It does not implement source-image replay, current-vs-historical Recipe policy, replay result persistence, live camera/motion/PLC/vision sequence execution, physical HMI acceptance, or real equipment validation.

## 2026-06-03 14:55 - Local validation passed
- Branch: `feature/fr-043-alarm-reset-boundary-scope`
- Scope: Add read-only AlarmView recovery boundary rows for operator acknowledgement, recovery memo persistence, hardware reset, PLC/vendor alarm source, and safety relay confirmation so the operator-visible Alarm Center does not imply unvalidated hardware reset coverage.
- Validation target: Alarm ViewModel test, App HMI XAML tests, Debug/Release build/test, static blocking/code-behind/secret scans, WPF launch smoke, GitHub Actions after PR creation.
- Local validation: App test project Debug build passed; Alarm/HMI targeted App tests passed with 11 tests. `dotnet restore .\VisionCell.sln`, Debug solution build, Debug solution test, Release solution build, and Release solution test passed. Debug and Release solution tests each passed with 258 tests. `git diff --check` passed with CRLF warnings only; precise blocking wait scan found no `.Result`, `.Wait()`, or `Thread.Sleep()` matches; WPF code-behind scan found no event-handler or blocking-call matches; narrow diff secret scan found no matches; WPF hidden launch smoke passed by starting `VisionCell.App.exe` for 5 seconds.
- Risks: This slice changes read-only Alarm Center ViewModel/XAML/docs only. It does not implement controller reset commands, PLC/vendor alarm ingestion, safety relay acknowledgement, field HMI acceptance, or real equipment validation.

## 2026-06-03 15:15 - Local validation passed
- Branch: `feature/fr-222-reinspect-persistence-history`
- Scope: Persist Offline Debug metadata Re-inspect comparison history in a dedicated SQLite table and display recent history rows in `OfflineDebugView` without writing new `inspection_results` replay rows.
- Validation target: Application Re-inspect use case tests, SQLite comparison repository tests, Offline Debug ViewModel/XAML tests, Debug/Release build/test, static blocking/code-behind/secret scans, WPF launch smoke, GitHub Actions after PR creation.
- Local validation: Application targeted tests passed with 3 tests; Persistence targeted tests passed with 4 tests after seeding the required source `inspection_results` row for the comparison FK; App targeted tests passed with 6 tests. `dotnet restore .\VisionCell.sln`, Debug solution build, Debug solution test, Release solution build, and Release solution test passed. Debug and Release solution tests each passed with 262 tests. `git diff --check` passed with CRLF warnings only; precise blocking wait scan found no `.Result`, `.Wait()`, or `Thread.Sleep()` matches; WPF code-behind scan found no event-handler or blocking-call matches; narrow diff secret scan found no matches; WPF hidden launch smoke passed by starting `VisionCell.App.exe` for 5 seconds.
- Risks: This slice persists metadata comparison history only. It does not implement source-image replay, current-vs-historical Recipe policy, new inspection-result replay persistence, live camera/motion/PLC/vision sequence execution, physical HMI acceptance, or real equipment validation.

## 2026-06-03 15:38 - Local validation passed with App Control workaround
- Branch: `feature/fr-222-reinspect-recipe-policy`
- Scope: Add an Application Re-inspect Recipe policy use case and Offline Debug UI bindings so Prepare Re-inspect shows whether the current active Recipe metadata matches, differs from, or is unavailable relative to the selected historical result.
- Validation target: Application Recipe policy tests, Offline Debug ViewModel/XAML tests, Debug/Release build/test, static blocking/code-behind/secret scans, WPF launch smoke, GitHub Actions after PR creation.
- Local validation: Application targeted tests passed with 4 tests. App targeted tests initially hit Windows Application Control on the generated Debug App test DLL, then passed with 5 tests after rebuilding `VisionCell.App.Tests` with `-p:Deterministic=false`. `dotnet restore .\VisionCell.sln`, Debug solution build, Release solution build, and Release solution test passed. Release solution test passed with 266 tests. Debug solution test ran non-App projects with 197 passing tests while App test discovery was blocked by Windows Application Control (`0x800711C7`); full Debug App tests then passed separately with 69 tests after the same non-deterministic rebuild workaround. `git diff --check` passed with CRLF warnings only; precise blocking wait scan found no `.Result`, `.Wait()`, or `Thread.Sleep()` matches; WPF code-behind scan found no event-handler or blocking-call matches; narrow diff secret scan found no matches; WPF hidden launch smoke passed by starting `VisionCell.App.exe` for 5 seconds.
- Risks: This slice resolves and displays active-vs-historical Recipe metadata only. It does not implement source-image replay, current-vs-historical replay execution, new inspection-result replay persistence, live camera/motion/PLC/vision sequence execution, physical HMI acceptance, or real equipment validation.

## 2026-06-03 16:09 - Local validation passed
- Branch: `feature/fr-222-source-image-replay-readiness`
- Scope: Add an Application Re-inspect source-image readiness use case and Offline Debug UI bindings so Prepare Re-inspect classifies camera-frame references, missing sources, unsupported URIs, unsafe traversal paths, and relative source replay candidates without opening files or executing replay.
- Validation target: Application source-image readiness tests, Offline Debug ViewModel/XAML tests, Debug/Release build/test, static blocking/code-behind/secret scans, WPF launch smoke, GitHub Actions after PR creation.
- Local validation: Application targeted tests passed with 5 tests; App targeted Offline Debug/HMI/App composition tests passed with 5 tests. `dotnet restore .\VisionCell.sln`, Debug solution build, Debug solution test, Release solution build, and Release solution test passed. Debug and Release solution tests each passed with 271 tests. `git diff --check` passed with CRLF warnings only; precise blocking wait scan found no `.Result`, `.Wait()`, or `Thread.Sleep()` matches; WPF code-behind scan found no event-handler or blocking-call matches; narrow diff secret scan found no matches; WPF hidden launch smoke passed by starting `VisionCell.App.exe` for 5 seconds.
- Risks: This slice classifies source-image replay readiness only. It does not archive source pixels, implement a source-image artifact reader, run current-vs-historical replay execution, persist new `inspection_results` replay rows, execute live camera/motion/PLC/vision sequences, perform physical HMI acceptance, or validate real equipment.

## 2026-06-03 16:33 - Local validation passed
- Branch: `feature/fr-222-source-frame-artifact-archive`
- Scope: Archive grabbed source frames as deterministic `.source.bmp` artifacts during Persist Result, store the source artifact path in `inspection_results.source_image_path`, and let Offline Debug source readiness report archived source BMP availability when artifact metadata is available.
- Validation target: Application inspection run/source readiness tests, Persistence artifact writer/SQLite result tests, Offline Debug ViewModel/App composition tests, Debug/Release build/test, static blocking/code-behind/secret scans, WPF launch smoke, GitHub Actions after PR creation.
- Local validation: Application targeted tests passed with 21 tests; Persistence targeted tests passed with 8 tests; App targeted tests passed with 5 tests. `dotnet restore .\VisionCell.sln`, Debug solution build, Debug solution test, Release solution build, and Release solution test passed. Debug and Release solution tests each passed with 275 tests. `git diff --check` passed with CRLF warnings only; precise blocking wait scan found no `.Result`, `.Wait()`, or `Thread.Sleep()` matches; WPF code-behind scan found no event-handler or blocking-call matches; narrow diff secret scan found no matches; WPF hidden launch smoke passed by starting `VisionCell.App.exe` for 5 seconds.
- Risks: This slice archives source BMP artifacts for newly generated inspection rows only. It does not implement source-image replay execution, customer image format loading, current-vs-historical replay execution, new `inspection_results` replay rows, live camera/motion/PLC/vision sequence replay, physical HMI acceptance, or real equipment validation.

## 2026-06-03 16:49 - Local validation passed
- Branch: `feature/fr-043-alarm-filter-triage`
- Scope: Add AlarmView active-only, severity, and equipment-area triage filters with visible-vs-total summary while keeping filtering in the ViewModel and leaving alarm persistence unchanged.
- Validation target: Alarm ViewModel filter tests, Alarm XAML binding tests, Debug/Release build/test, static blocking/code-behind/secret scans, WPF launch smoke, GitHub Actions after PR creation.
- Local validation: App targeted Alarm tests passed with 3 tests. `dotnet restore .\VisionCell.sln`, Debug solution build, Debug solution test, Release solution build, and Release solution test passed. Debug and Release solution tests each passed with 275 tests. `git diff --check` passed with CRLF warnings only; precise blocking wait scan found no `.Result`, `.Wait()`, or `Thread.Sleep()` matches; WPF code-behind scan found no event-handler or blocking-call matches; narrow diff secret scan found no matches; WPF hidden launch smoke passed by starting `VisionCell.App.exe` for 5 seconds.
- Risks: This slice changes Alarm Center triage UI/ViewModel behavior only. It does not add SQL-level filtering, real PLC/vendor alarm sources, controller reset commands, safety relay acknowledgement, field HMI acceptance, or real equipment validation.

## 2026-06-03 16:37 - Local validation passed
- Branch: `feature/fr-240-adapter-readiness-matrix`
- Scope: Add a read-only hardware adapter boundary catalog and Settings adapter matrix for Motion Controller, Camera, and PLC I/O roles so current simulator providers, future adapter contracts, and missing bench evidence are visible without enabling real hardware.
- Validation target: Equipment adapter catalog tests, Settings ViewModel/XAML tests, Debug/Release build/test, static blocking/code-behind/secret scans, WPF launch smoke, GitHub Actions after PR creation.
- Local validation: Equipment targeted adapter tests passed with 3 tests; App targeted Settings/HMI tests passed with 2 tests. `dotnet restore .\VisionCell.sln`, Debug solution build, Debug solution test, Release solution build, and Release solution test passed. Debug and Release solution tests each passed with 276 tests. `git diff --check` passed with CRLF warnings only; precise blocking wait scan found no `.Result`, `.Wait()`, or `Thread.Sleep()` matches; WPF code-behind scan found no event-handler or blocking-call matches; narrow diff secret scan found no matches; WPF hidden launch smoke passed by starting the WPF app for 5 seconds.
- Risks: This slice changes read-only hardware adapter readiness catalog, Settings UI, tests, and docs only. It does not implement `RealEquipmentController`, vendor SDK wrappers, PLC protocol, fieldbus communication, camera trigger integration, safety relay reset, persisted Settings editing, bench validation, physical HMI acceptance, or real equipment validation.

## 2026-06-03 16:47 - Local validation passed
- Branch: `feature/fr-084-fault-interlock-impact`
- Scope: Add read-only Equipment interlock impact rows for Servo On, Home, Jog, Move Absolute, Enter Auto, Run Inspection, and Reset Alarm so simulator fault injection visibly connects I/O fault state to backend command blocking.
- Validation target: Equipment ViewModel fault-injection/interlock tests, Equipment XAML marker tests, Debug/Release build/test, static blocking/code-behind/credential scans, WPF launch smoke, GitHub Actions after PR creation.
- Local validation: App test project Debug build passed; Equipment fault-injection/HMI targeted App tests passed with 3 tests. `dotnet restore .\VisionCell.sln`, Debug solution build, Debug solution test, Release solution build, and Release solution test passed. Debug and Release solution tests each passed with 276 tests. `git diff --check` passed with CRLF warnings only; precise blocking wait scan found no `.Result`, `.Wait()`, or `Thread.Sleep()` matches; WPF code-behind scan found no event-handler or blocking-call matches; diff credential-pattern scan found no matches; WPF hidden launch smoke passed by starting the WPF app for 5 seconds.
- Risks: This slice changes simulator-backed Equipment ViewModel/XAML interlock impact display, tests, and docs only. It does not implement real PLC scan polling, output writes, safety relay response, hardware reset confirmation, physical HMI acceptance, or real equipment validation.

## 2026-06-03 16:59 - Local validation passed
- Branch: `feature/fr-006-hmi-codebehind-qa-guard`
- Scope: Add automated App XAML QA guards for priority HMI screen ScrollViewer/`Grid MinWidth=0` layout structure and module code-behind initialization-only behavior.
- Validation target: App HMI XAML QA tests, Debug/Release build/test, static blocking/code-behind/credential scans, WPF launch smoke, GitHub Actions after PR creation.
- Local validation: App test project Debug build passed after replacing expression-tree null-propagation assertions with projected values. Targeted HMI QA tests passed with 3 tests, then the rebuilt code-behind/layout guard tests passed with 2 tests. `dotnet restore .\VisionCell.sln`, Debug solution build, Debug solution test, Release solution build, and Release solution test passed. Debug and Release solution tests each passed with 278 tests. `git diff --check` passed with CRLF warnings only; precise blocking wait scan found no `.Result`, `.Wait()`, or `Thread.Sleep()` matches after splitting QA guard string literals; WPF code-behind scan found no event-handler or blocking-call matches; diff credential-pattern scan found no matches; WPF hidden launch smoke passed by starting the WPF app for 5 seconds.
- Risks: This slice adds automated XAML/code-behind QA guards and docs only. It does not complete physical 1920x1080 or 1366x768 HMI panel validation, touch operation validation, real equipment workflow validation, or operator acceptance testing.

## 2026-06-03 17:09 - Local validation passed with App Control workaround
- Branch: `feature/fr-006-long-text-grid-wrapping`
- Scope: Convert selected long diagnostic GridView columns to wrapping text templates across Motion, Equipment, Recipe, Offline Debug, and Alarm screens, and add an App XAML QA test to keep those columns wrapping.
- Validation target: App HMI XAML long-text wrapping tests, Debug/Release build/test, static blocking/code-behind/credential scans, WPF launch smoke, GitHub Actions after PR creation.
- Local validation: App test project Debug build passed after replacing a method-group FluentAssertions predicate with a lambda. Targeted long-text/HMI App tests passed with 2 tests. Initial `dotnet test .\VisionCell.sln -c Debug --no-build` was blocked by local Windows App Control (`0x800711C7`) on generated Debug DLLs such as `VisionCell.Core.dll` and `VisionCell.Vision.dll`; rebuilding Debug with `-p:Deterministic=false` passed, and the rerun Debug solution test passed with 281 tests. `dotnet restore .\VisionCell.sln`, initial Debug solution build, Release solution build, and Release solution test passed; Release solution test passed with 281 tests. `git diff --check` passed with CRLF warnings only; precise blocking wait scan found no `.Result`, `.Wait()`, or `Thread.Sleep()` matches; WPF code-behind scan found no event-handler or blocking-call matches; diff credential-pattern scan found no matches; WPF hidden launch smoke passed by starting the WPF app for 5 seconds.
- Risks: This slice changes XAML long-text presentation, App XAML tests, and docs only. It does not validate physical panel readability, touch operation, operator acceptance, or real equipment workflow.

## 2026-06-04 20:05 - Local validation passed
- Branch: `feature/fr-241-error-code-catalog`
- Scope: Add a Core `ErrorCodeCatalog`, route `EquipmentAlarmFactory` and selected-alarm recovery hints through the shared catalog, and surface a read-only AlarmView error-code catalog with code, severity, area, cause, and recovery action.
- Validation target: Core error-code catalog tests, AlarmViewModel/XAML tests, Debug/Release build/test, static blocking/code-behind/sensitive-value scans, WPF launch smoke, GitHub Actions after PR creation.
- Local validation: Core targeted catalog/factory tests passed with 2 tests; App targeted Alarm/HMI tests passed with 16 tests. Initial parallel App test project build hit a transient Core `obj` file lock from a concurrent Core build; the App test project build passed when rerun serially. `dotnet restore .\VisionCell.sln`, Debug solution build, Debug solution test, Release solution build, and Release solution test passed. Debug and Release solution tests each passed with 280 tests. `git diff --check` passed with CRLF warnings only; precise blocking wait scan found no `.Result`, `.Wait()`, or `Thread.Sleep()` matches; WPF code-behind scan found no event-handler or blocking-call matches; diff sensitive-value scan found no matches; WPF hidden launch smoke passed by starting `VisionCell.App.exe` for 5 seconds.
- Risks: This slice centralizes and displays static protocol error-code guidance only. It does not implement real PLC/vendor alarm-source ingestion, controller reset commands, safety relay acknowledgement, hardware reset confirmation, physical HMI acceptance, or real equipment validation.

## 2026-06-04 20:16 - Local validation passed
- Branch: `feature/fr-006-star-column-layout-guard`
- Scope: Add `MinWidth=0` to star `ColumnDefinition` entries in priority HMI module layouts and add an App XAML QA guard so long tables/status text cannot force horizontal workspace growth.
- Validation target: App HMI XAML tests, Debug/Release build/test, static blocking/code-behind/sensitive-value scans, WPF launch smoke, GitHub Actions after PR creation.
- Local validation: App targeted HMI XAML tests passed with 14 tests. `dotnet restore .\VisionCell.sln`, Debug solution build, Debug solution test, Release solution build, and Release solution test passed. Debug and Release solution tests each passed with 281 tests. `git diff --check` passed with CRLF warnings only; precise blocking wait scan found no `.Result`, `.Wait()`, or `Thread.Sleep()` matches; WPF code-behind scan found no event-handler or blocking-call matches; diff sensitive-value scan found no matches; WPF hidden launch smoke passed by starting `VisionCell.App.exe` for 5 seconds.
- Risks: This slice changes XAML layout guards and App XAML tests only. It does not validate physical 1920x1080 or 1366x768 panels, touch operation, operator acceptance, or real equipment workflow.

## 2026-06-04 20:27 - Local validation passed
- Branch: `feature/fr-006-inspection-kpi-cards`
- Scope: Replace InspectionView one-off summary panels with shared `KpiCard` controls and add an App XAML QA guard for the Inspection summary band.
- Validation target: App HMI XAML tests, Debug/Release build/test, static blocking/code-behind/sensitive-value scans, WPF launch smoke, GitHub Actions after PR creation.
- Local validation: App targeted HMI XAML tests passed with 15 tests. `dotnet restore .\VisionCell.sln`, Debug solution build, Debug solution test, Release solution build, and Release solution test passed. Debug and Release solution tests each passed with 282 tests. `git diff --check` passed with CRLF warnings only; precise blocking wait scan found no `.Result`, `.Wait()`, or `Thread.Sleep()` matches; WPF code-behind scan found no event-handler or blocking-call matches; diff sensitive-value scan found no matches; WPF hidden launch smoke passed by starting `VisionCell.App.exe` for 5 seconds.
- Risks: This slice changes Inspection XAML presentation and App XAML tests only. It does not change inspection sequence behavior, camera/motion/vision execution, physical HMI acceptance, touch operation, or real equipment validation.

## 2026-06-04 20:36 - Local validation passed
- Branch: `feature/fr-006-dashboard-kpi-cards`
- Scope: Replace Dashboard overview one-off panels with shared `KpiCard` controls and add an App XAML QA guard for the Dashboard overview band.
- Validation target: App HMI XAML tests, Debug/Release build/test, static blocking/code-behind/sensitive-value scans, WPF launch smoke, GitHub Actions after PR creation.
- Local validation: App targeted HMI XAML tests passed with 16 tests. `dotnet restore .\VisionCell.sln`, Debug solution build, Debug solution test, Release solution build, and Release solution test passed. Debug and Release solution tests each passed with 283 tests. `git diff --check` passed with CRLF warnings only; precise blocking wait scan found no `.Result`, `.Wait()`, or `Thread.Sleep()` matches; WPF code-behind scan found no event-handler or blocking-call matches; diff sensitive-value scan found no matches; WPF hidden launch smoke passed by starting `VisionCell.App.exe` for 5 seconds.
- Risks: This slice changes Dashboard XAML presentation and App XAML tests only. It does not change equipment controller behavior, command interlocks, simulator state, physical HMI acceptance, touch operation, or real equipment validation.
