...
- Artifact generation now flows through Application `IInspectionArtifactWriter` and Persistence `FileSystemInspectionArtifactWriter`, creating deterministic source, overlay, and height-map BMP files during Persist Result.
- Offline Debug now reads persisted inspection results through Application `IInspectionResultReader`, artifact metadata/preview pixels through `IInspectionArtifactReader`, source-image replay readiness through `IInspectionReinspectSourceImageReadinessUseCase`, active-vs-historical Recipe policy through `IInspectionReinspectRecipePolicyUseCase`, and persisted metadata Re-inspect comparison history through `IInspectionReinspectComparisonReader`, displaying recent result metadata, defect rows, correlation IDs, artifact paths, artifact state, overlay preview, height-map preview, source replay readiness, Recipe policy metadata, and metadata comparison history without executing live equipment replay.
...
- `ErrorCodeCatalog` centralizes documented alarm/error code area, severity, cause, and recovery-action mappings for both `EquipmentAlarmFactory` and the WPF Alarm Center catalog.
...
- I/O transition history now uses `IEquipmentIoTransitionRepository` and `SqliteEquipmentIoTransitionRepository` so simulator fault injection can persist changed I/O bit value/forced-state records without WPF database writes.
