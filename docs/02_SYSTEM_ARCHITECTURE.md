...
- Offline Debug now reads persisted inspection results through Application `IInspectionResultReader`, artifact metadata/preview pixels through `IInspectionArtifactReader`, active-vs-historical Recipe policy through `IInspectionReinspectRecipePolicyUseCase`, and persisted metadata Re-inspect comparison history through `IInspectionReinspectComparisonReader`, displaying recent result metadata, defect rows, correlation IDs, artifact paths, artifact state, overlay preview, height-map preview, Recipe policy metadata, and metadata comparison history without executing live equipment replay.
