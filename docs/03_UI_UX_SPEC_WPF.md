...
- The Re-inspect panel shows read-only readiness rows for metadata comparison, source-image replay, Recipe policy, metadata history persistence, and real sequence execution so unimplemented or unvalidated replay boundaries remain operator-visible.
...
- Source-image replay, current-vs-historical Recipe policy, new inspection-result replay persistence, and actual camera/motion/vision sequence execution remain follow-up work.
