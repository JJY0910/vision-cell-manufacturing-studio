...
- Offline Debug validation covers SQLite result reads, relative artifact metadata checks, deterministic BMP preview decoding, safe external artifact open preparation, Re-inspect preparation state, read-only Re-inspect readiness boundary rows, Application metadata comparison results, and SQLite metadata comparison history persistence.
...
- Offline Debug Re-inspect currently runs and persists deterministic metadata comparisons over the prepared historical result context. Source-image replay, current-vs-historical Recipe resolution, new `inspection_results` replay result persistence, and real camera/motion/vision sequence execution are not implemented or validated.
