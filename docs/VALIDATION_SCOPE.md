...
- UI QA validation covers local WPF build/runtime loading, shared XAML resource resolution, ViewModel regression tests, static code-behind scans, and a five-second WPF launch smoke. The current HMI polish pass covers layout reachability, command tooltips, wrapping command forms, scrollable operator tables, Shell status chips, dark Event Log styling, and reusable `AxisCard` / `SequenceTimeline` binding only in the local developer environment.
