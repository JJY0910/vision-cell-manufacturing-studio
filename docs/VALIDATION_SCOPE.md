...
- Offline Debug Re-inspect currently classifies source-image replay readiness, resolves active-vs-historical Recipe metadata, then runs and persists deterministic metadata comparisons over the prepared historical result context. Source pixel archival, source-image replay execution, current-vs-historical replay execution, new `inspection_results` replay result persistence, and real camera/motion/vision sequence execution are not implemented or validated.
