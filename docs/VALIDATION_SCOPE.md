...
- Offline Debug validation covers SQLite result reads, relative artifact metadata checks, deterministic BMP preview decoding, safe external artifact open preparation, Re-inspect preparation state, read-only Re-inspect readiness boundary rows, active-vs-historical Recipe policy metadata, Application metadata comparison results, and SQLite metadata comparison history persistence.
...
- Offline Debug Re-inspect currently resolves active-vs-historical Recipe metadata, then runs and persists deterministic metadata comparisons over the prepared historical result context. Source-image replay, current-vs-historical replay execution, new `inspection_results` replay result persistence, and real camera/motion/vision sequence execution are not implemented or validated.
