...
- I/O Monitor and Fault Injection validation covers `VirtualEquipmentController`, Application fault injection use case, WPF `EquipmentViewModel`, active fault and forced I/O summaries, simulator I/O forced-state rows, read-only interlock impact rows, read-only I/O transition history binding, interlock blocking, Application alarm recorder calls, and SQLite persistence of simulator I/O transition history.
- UI QA validation covers local WPF build/runtime loading, shared XAML resource resolution, ViewModel regression tests, App XAML layout guard tests, star-column `MinWidth=0` layout guard tests, shared Dashboard/Inspection `KpiCard` guard tests, long-text GridView wrapping tests, module code-behind initialization-only tests, static code-behind scans, and a five-second WPF launch smoke. The current HMI polish pass covers layout reachability, command tooltips, shared Dashboard/Equipment/Motion/Teaching/Recipe/Inspection/Alarm command headers, shared CommandBar action button styling, shared secondary action button styling, shared compact Equipment fault-injection buttons, wrapping command forms, scrollable operator tables, Shell status chips, dark Event Log styling, Settings read-only Real Hardware readiness gate visibility, and reusable `KpiCard` / `AxisCard` / `SequenceTimeline` / `IoBitIndicator` / `ImageViewport` / `RoiOverlayCanvas` / `ErrorBanner` / `CommandBar` / `RecipeEditorField` binding only in the local developer environment.
...
- Fault interlock impact rows are derived from simulator snapshots and Application interlock rules only; they do not validate real PLC scan timing, safety relay response, or hardware reset confirmation.
